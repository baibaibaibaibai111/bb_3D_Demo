...
function tickNeeds(delta, interactionState, sleepingWithLightOn) {
  ...
  if (interactionState === "eat_food" || interactionState === "fridge_eat") {
...
function getInteractionPreferenceScore(actionId) {
  ...
  let key = actionId;
  if (actionId === "fridge_eat") key = "eat_food";
  else if (
    actionId === "tv_watch" ||
    actionId === "sofa_sit" ||
    actionId === "pillow_fight" ||
    actionId === "play_guitar" ||
    actionId === "use_computer"
  ) {
    key = "tv_watch";
  } else if (
    actionId === "sink_wash" ||
    actionId === "shower_wash" ||
    actionId === "bathtub_bath"
  ) {
    key = "wash_sink";
  }

  const v = personality.interactionPreference[key];
